...
序盤はCostomLayoutってなんだっけっていうのを確認して、

後半では、DailyScheduerの作成を通してCustomLayoutへの理解をより深めます。
-->
...
セッションの感想やご意見はぜひDroidkaigiのタグをつけてXに投稿していただければと思います。
Xに投稿するところまで含めてDroidkaigiだと思っておりますので何卒よろしくお願いいたします。
その際#Layoutというタグもつけていただけると、このセッションのことだとわかるのでさらにありがたいです。
...
駆け足で紹介するので、詳細よりもどれをどう選べばいいかをメインにおさえていただければと思います。
...
→なので、Layout関数でできることに加えて何を配置するかまでLayoutフェーズで変えたい場合にはSubCopmoseLayoutを使う必要があります。
...
ここではtimeLabelひとつひとつに対応したMesurableがListになって入ってきます。
...
minハイトには0、maxには一時間あたりの高さを指定してやります。
...
今回は縦に順にずらしていきたいので、一時間あたりの高さにindexをかけたものをyに入れています。
...
<!--
コード的にはこんな感じで、説明は省きますが、重なっているものが同じListに入るListのListを作っています。
...
offsetの更新はイベントの更新よりも高頻度なため、余計なreCompositionを防ぐため別で持たせた方が良いです。
-->
...
<!--
準備ができたので、eventのcontentにDraggableをつけます。->
...
<!--
最後にonDragストップどで、DragStateとyOffsetを元に戻します。
...
<!--
これでlayout中にドラッグの状態が取れるようになったので、drag中のイベントだった場合にのみ、offSetぶんずらしてやるようにします。
...
<!--
ドラッグ中のアイテムの見た目の更新はスナッピングの後にやります。
...
ドラッグしたアイテムがカクカクとする動きですね。今回は15の倍数の分にスナップしてみます

やることは簡単で、ドラッグ中にoffsetを単純に反映した位置ではなく、一番近いきりのいい位置に起くだけです。
...
ここで、CompositionPhaseで参照しているデータを変更しているのが若干不安でして、他に方法も思いつかないのと、特に問題が起きていないのでそのままにしているんですが、
...
ただ、見た目を更新しているのはドラッグ中のeventだけなので、他のイベントのRecompositionは必要がないはずです。
...
なので遅延レンダリングというよりはLazyColmunのようなことをやりたい、というとわかりやすいかもしれません。
...
単純に365日分表示してみると、アウトオブメモリでクラッシュしてしまいます。
...
<!--
まずは画面に表示できる個数の計算です。
scrollStateからviewPort、Viewの表示サイズが取れるので、それを一時間あたりの高さで割って表示する時刻の数を求めます。
...
できそうとかできなそうとかもぜひXに呟いていただけると後でニヤニヤしながら見れますのでぜひよろしくお願いいたします。
-->
