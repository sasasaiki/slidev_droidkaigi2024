...
下二つをCustomLayoutと呼ぶかは微妙なのですが、用途としては近いものがあるので合わせて見ていきます。

それぞれがどういった特徴を持っているのかは最初の迷いどころだと思うので、確認しておきましょう。
-->
...
### レイアウトフェーズで複数の要素の位置とサイズをカスタマイズしたい
...
この関数を使うことでレイアウトフェーズを独自に実装したコンポーザブルを作成することができます。

要素一つだけではなく複数の子要素を扱うことができ、親の制約や、他の要素のサイズを参照することが可能です。

なので、親の制約や他の要素のサイズといった値を見てから、要素の配置を確定することができます。
...
measurrablesに三つの要素が入ってくるため、text1,2,3それぞれに対して自由な配置をすることが可能です。
...
### レイアウトフェーズで単一の要素の位置とサイズをカスタマイズしたい


<!--
次に二つ目のmodifier.layout()
です。Layout関数とかなり使いのですが、こちらはModifierの関数なので、ComposableのModifierで呼び出し、modifierが付けられたCompossableの子要素のレイアウトに影響を与えるというのと、複数の要素を扱うことはできない、という点が大きな違いになります。
...
具体的に見てみるとわかりやすいのですが、modifer.呼び出して使います。

そしてこのbloackの中でサイズや位置の調整を行います。
ColmunにはTextが二つ入っていますが、このブロックにはテキストを二つ並べて、まとめて一つとみなしたmesurabaleしか入ってこないので、
text1は文字幅いっぱいに伸ばして、text2は親の幅いっぱいに伸ばす、みたいな実装はできず、text1とtext2を両方右にずらすといった使い方ができます。
...
これをCostomLayoutと呼ぶかは相当微妙なのですが、Layoutについて調べてると出てくることも多いので紹介します。
...
<!--
これは親のmaxWidthが600.dpより大きいか小さいかでシンプルに、親の制約によってなかの要素を変えたい場合に利用ができます。
...
ラベル、イベントなど複数の要素があり、
それぞれ位置とサイズの調整はしますが、場合によってcontentを変えることはしないので複数の要素を自由に配置するLayoutを使っていきます。
-->
...
引数にはcontentとmodifierとmeasurePolicyを受け取ります。メジャーポリシーは末尾にラムダでかけますが一応わかりやすく書いています。
...
layoutを始める前にこのメジャーポリシーのブロックに何が渡されてくるのかを確認しましょう
...
何もなかった場合は、minが0,maxがそろぞれ表示領域の高さになり、->
...
で、もう一つ、minとmaxにInfinityが入ってしまった場合にもクラッシュします。
...
確認して見たところ、その場合はmodifierに何もついていない時と同じ挙動、Textの場合はwrapContentのような挙動になるので
...
alignmentLinesは今回使わないので割愛するのですが、知っておくと便利なので時間がある時にチェックして見てください。
