...
<!--
fuga20:ここから
ではまず、CostomLayoutとは何かという話をしていきましょう。
...
measureの結果はPlaceableというクラスで返ってきます。このPlaceeableから高さや幅を取得することが可能です。例えばこのラベルと同じサイズの何かを作りたい場合にもPlaceable.haitを参照することで実現ができます。
...
alignmentLinesは今回使わないのですごく軽く説明するのですが、これを使うことで、作成したCustomLayoutを利用するComposable、親の直接的な親および間接的な親のLayoutフェーズに任意のIntを渡すことが可能です。
...
ブロックに入ってきたmesurabeはeventの情報を持っていないので、開始時刻と終了時刻をmeasureのblock内で取ってくることができません。
...
そうしましたら、グルーピングしてわかった重なるイベントの数と位置をEventに持たせます。
